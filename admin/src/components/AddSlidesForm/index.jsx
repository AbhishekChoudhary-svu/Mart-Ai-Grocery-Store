...
const AddSlidesForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = isEditMode
      ? `/api/homeSlides/update/${id}`
      : `/api/homeSlides/AddSlides`;
  ...
  const removeImage = async (imgUrl, index) => {
    const res = await deleteImages(
      `/api/homeSlides/delete-Slides-Image?img=${encodeURIComponent(imgUrl)}`
    );
    if (res?.data?.success) {
  ...
  return ( 
    <>
      ...
      <h2 className="md:text-2xl text-xl font-semibold mb-4">{isEditMode ? "Edit Home Slides Banner" : "Add Home Slides Banner"}</h2>
      <form
      ...
      >
        <div className="md:flex-row flex flex-col gap-4">
          <div className="col1 flex flex-col gap-4 md:w-[50%] w-full">
            ...
            {/* Banner Id */}
            <div>
              <label className="block font-medium">Banner Id (Not Required)</label>
              <input
                type="text"
                name="id"
                className="w-full mt-1 p-2 border border-gray-300 rounded"
          ...
          <div className="col2 md:w-[50%] w-full">
            ...
            <div
              className="relative flex flex-col items-center justify-center border-2 border-dashed border-gray-400 rounded-md p-5 cursor-pointer hover:border-black transition"
            >
              ...
              <input
                ...
                multiple
               onChange={(e) =>
                    onChangeFile(e, "/api/homeSlides/upload-Slides-images")
                  }
                accept="image/*"
        ...
        >
          {isEditMode ? "Update Banner Slides" : "Add Banner Slides"}  
        </button>
      </form>
    </div>
