...
const ProductListing = () => {
  ...
  const [breadName, setBreadName] = useState({ catName: "", subCatName: "" });
  ...
  useEffect(() => {
    if (productData.length > 0) {
      ...
      setBreadName({
        ...
        subCatName: firstProduct.subCatName || "",
      });
    }
  }, [productData]);
    ...
    <section className="py-5 pb-0">
      <div className="container">
        <Breadcrumbs aria-label="breadcrumb">
          ...
          {breadName.catName && <span>{breadName.catName}</span>}
          {breadName.subCatName && <span>{breadName.subCatName}</span>}
        </Breadcrumbs>
      </div>
      <div className="bg-white mt-3 py-4">
        <div className="container flex gap-3">
          <div
            className={` lg:w-[20%] w-full  lg:h-full fixed  left-0 lg:static    -bottom-[100%] z-[77] lg:z-0 p-5 pr-0 py-3 lg:py-5 lg:p-0 bg-white  transition-all lg:opacity-100 opacity-0 ${
              context.openFilters === true && "sidebarWrapper"
            } `}
          >
            <SideBar
          ...
          {context.windowWidth < 992 && (
            <div
              onClick={() => context.setOpenFilters(false)}
              className={`fixed inset-0 bg-black/40 z-[68] ${
                context.openFilters === true ? "block" : "hidden"
              } `}
            ></div>
          )}
